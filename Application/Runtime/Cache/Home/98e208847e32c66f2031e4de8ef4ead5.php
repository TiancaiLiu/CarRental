...
<ul class="nav">
  ...
  <?php if(is_array($cates)): foreach($cates as $key=>$v): ?><li>
   	 <a href="/Home/<?php if($v['id'] == 1): ?>Aboutus<?php elseif($v['id'] == 2): ?>Intro<?php elseif($v['id'] == 3): ?>Rental<?php elseif($v['id'] == 4): ?>Return<?php elseif($v['id'] == 5): ?>Money<?php elseif($v['id'] == 6): ?>Activity<?php elseif($v['id'] == 7): ?>Carlist<?php elseif($v['id'] == 8): ?>Maintain<?php elseif($v['id'] == 9): ?>Contact<?php endif; ?>/index/cateid/<?php echo ($v["id"]); ?>" title="关于我们">
   	 <?php echo ($v["catename"]); ?>
   	 </a>
...
<div class="content">
    <div class="product-top">
        <p>共<font><?php echo ($count); ?></font>条信息</p>
    </div>
...
<script type="text/javascript">
    ...
    function getlist(p) {
        $.ajax({
            type:"POST",
            url:"/Home/Search/getList/", 
            data:{
                p:1,
                kws:'<?php echo I('post.kws');?>',
            },
            dataType:"json",
            success:function(data) {
                ...
                $(data).each(function(index, el) {
                    content += '<li class="news4"><a href="/Home/Cardetail/index/id/'+el.id+'"><img src="/'+el.pic+'" /><span>'+el.title+'</span></a></li>';
                });
